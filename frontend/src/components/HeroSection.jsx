...
const HeroSection = () => {
  ...
  return (
    <section id="home" className="min-h-screen w-full flex flex-col justify-center items-center text-center px-4 relative overflow-hidden transition-colors duration-500 bg-slate-50 dark:bg-[#070b14]" style={{ display: 'flex', position: 'relative', zIndex: 1 }}>
      {/* Yin-Yang Mesh Gradient — Two halves rotating as one */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none" style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      ...
      >
        ...
        <motion.div initial="visible" variants={itemVariants} className="space-y-4 mb-6 relative z-20" style={{ position: 'relative', zIndex: 20 }}>
          ...
          <p className="text-xl md:text-2xl font-light text-slate-700 dark:text-white/80 transition-colors duration-500" style={{ fontSize: '1.25rem', color: '#374151' }}>
            Backend  Developer <span className="text-fuchsia-500 mx-2" style={{ color: '#d946ef' }}>|</span> Technology Enthusiast
          </p>
        </motion.div>
